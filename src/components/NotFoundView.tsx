...
export function NotFoundView() {
  return (
    <section className="mx-auto max-w-[600px] px-[18px] py-[130px] pb-[150px] text-center sm:px-8">
      ...
      <h1 className="mt-1.5 font-heading text-2xl font-extrabold">{NOT_FOUND.title}</h1>
      <p className="mt-3 text-[15.5px] text-muted-2">{NOT_FOUND.body}</p>
      <div className="mt-8 flex flex-wrap justify-center gap-3.5">
        <LinkButton href="/">{NOT_FOUND.ctaHome}</LinkButton>
        <LinkButton href="/products" variant="ghost-dark">
          {NOT_FOUND.ctaProducts}
        </LinkButton>
      </div>
    </section>
  );
