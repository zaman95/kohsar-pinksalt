...
export function ProductGallery({ images, name, tag }: { images: SanityImageRef[]; name: string; tag?: string }) {
  ...
  return (
    <div>
      <div className="relative h-[320px] overflow-hidden rounded-[22px] bg-slot sm:h-[400px] lg:h-[460px]">
        <SanityImage image={gallery[active]} alt={`${name} — main product photo`} priority />
        {tag && (
      ...
      {gallery.length > 1 && (
        <div className="mt-4 grid grid-cols-4 gap-3.5">
          {gallery.map((img, i) => (
            ...
            >
              <SanityImage image={img} alt={`${name} thumbnail ${i + 1}`} />
            </button>
          ))}
        </div>
      )}
